...
import sys
sys.path.append("/home/pratikaher/GNNIE/GNN_Architecture/")
...
model_config = load_config(sys.path[0] + "/../model_config.yml")

graphs, _ = dgl.load_graphs(f"{BASE_DIR}/graph_files_vmcloud/ecommerce_hetero_graph.dgl")
ecommerce_hetero_graph = graphs[0]

graphs, _ = dgl.load_graphs(f"{BASE_DIR}/graph_files_vmcloud/train_g.dgl")
train_g = graphs[0]

graphs, _ = dgl.load_graphs(f"{BASE_DIR}/graph_files_vmcloud/test_g.dgl")
test_g = graphs[0]

graphs, _ = dgl.load_graphs(f"{BASE_DIR}/graph_files_vmcloud/valid_g.dgl")
valid_g = graphs[0]

dim_dict = {
    'customer': ecommerce_hetero_graph.nodes['customer'].data['features'].shape[1],
...
mpnn_model = ConvModel(ecommerce_hetero_graph, model_config['num_layers'], dim_dict, aggregator_type=model_config['aggregate_fn'], pred=model_config['pred'])
...
edge_sampler = dgl.dataloading.EdgePredictionSampler(
    node_sampler, negative_sampler=neg_sampler, exclude='self'
)
...
y = {
    ntype: torch.zeros(valid_g.num_nodes(ntype), 64) for ntype in valid_g.ntypes
}
...
for arg0, pos_g, neg_g, blocks in valid_dataloader:
    ...
    input_features['customer'] = mpnn_model.user_embed(
    ...
    for ntype in h.keys():
        y[ntype][output_nodes[ntype]] = h[ntype].detach()
...
def create_already_rated(g):
    ...
    already_rated_arr = np.stack(
        (np.asarray(customers_train), np.asarray(product_train)), axis=1
    )
...
def compare_rec(valid_recs, model_recs):
    ...
    for key, value in model_recs.items():
        ...
        recommended_movies_correct = set(
            list(set(model_recs_list) & set(test_recs_list))
        )
